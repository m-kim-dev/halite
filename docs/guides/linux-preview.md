...
Open a Markdown document and use **Source** below the navigation bar to see its
original markup. **Preview** returns to the rendered document. Both views keep
their own reading position while this reader stays open.

Use **Copy Markdown** in either view to copy the whole document, including its
links, formatting, indentation, and code fences. **Copied!** confirms success.
If clipboard access is unavailable, Halite selects the source so you can press
Ctrl+C or ⌘C. Manual selection follows the browser's normal text-copy behavior.

Source is read-only and refreshes when you save in your editor. Switching project
tabs keeps the selected view. Navigating to a document, reloading, or moving the
project to another window starts in Preview. Ctrl+F can search Source in the
desktop app; the find panel closes when switching views.

For email replies that should have no Markdown formatting, open the plain-text
version and use its **Copy** button instead.
