...
The six new browser cases cover literal Markdown and CRLF copy payloads,
clipboard failure and complete source selection, separate view positions,
navigation/reload, live edits, empty files, stale clipboard promises, controls
disabled during document loading, keyboard access, and a 320-pixel layout.
The source view was visually inspected in the desktop and narrow browser layouts.

Source-app and portable-archive desktop checks passed using the real system
clipboard. They also verified live source refresh, view retention across project
tabs, and native find in both views. The CLI and eight-project browser workspace
checks passed, including one shared process/port and retained reading positions.
No backend or persisted-preference migration was needed for this feature.

The 0.3.0 `.deb` passed sandboxed desktop workflows in Debian 12 and Ubuntu
24.04.4 x64 containers, before and after a synthetic package revision upgrade.
Removal preserved external state and project files. An initial Ubuntu check
read the clipboard before its asynchronous write finished; the test now waits
for the app's success feedback before asserting the actual clipboard contents.
The full Ubuntu workflow then passed. Both release artifact checksums passed.

Source-app and host-archive checks used the explicit test-only sandbox override;
the installed `.deb` checks kept Chromium sandboxing enabled. Container checks
use a virtual display and mocked native picker selections. They do not establish
real Wayland/macOS/Windows/ARM desktop compatibility or migration from an older
released application. Earlier records below describe their historical builds.
